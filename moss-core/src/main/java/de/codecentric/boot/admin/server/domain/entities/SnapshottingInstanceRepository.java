...
public class SnapshottingInstanceRepository extends EventsourcingInstanceRepository {
    ...
    public Flux<Instance> findByName(String name) {
        return this.findAll().filter(a -> a.getRegistration().getName().toLowerCase().contains(name.toLowerCase()));
    }
    ...
    public void revomeInstance(InstanceId instanceId){
        snapshots.remove(instanceId);
    }

    public InstanceEventStore getHaloEventStore() {
        return this.getEventStore();
    }
}
